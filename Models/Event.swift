...
public struct Event: Codable, Hashable, Identifiable {
  ...
  static let shortDayDF: DateFormatter = {
    let df = DateFormatter()
    df.dateStyle = .short
  ...
  public var shortDateDisplay: String {
    let dateDisplay = Event.shortDayDF.string(from: start)
    return dateDisplay
  }
...
extension Event {
  public static let debug = Event(id: 0, title: "A Very, Very, Very Long Event Title", summary: "Event Summary", eventDescription: "Event Description", eventHTML: nil, links: [], venue: .debug, venueDetails: "Venue Details", published: nil, start: Date(), end: Date().addingTimeInterval(60*60), tags: [])
}
